...
const oauthSchema = z.object({
  ...
  MicrosoftOAuthEnabled: z.boolean(),
  MicrosoftClientId: z.string(),
  MicrosoftClientSecret: z.string(),
  discord: z.object({
    enabled: z.boolean(),
    client_id: z.string(),
...
const buildFormDefaults = (defaults: FlatOAuthDefaults): OAuthFormValues => ({
  ...
  MicrosoftOAuthEnabled: defaults.MicrosoftOAuthEnabled,
  MicrosoftClientId: defaults.MicrosoftClientId ?? '',
  MicrosoftClientSecret: defaults.MicrosoftClientSecret ?? '',
  discord: {
...
const normalizeFormValues = (values: OAuthFormValues): FlatOAuthDefaults => ({
  ...
  MicrosoftOAuthEnabled: values.MicrosoftOAuthEnabled,
  MicrosoftClientId: values.MicrosoftClientId,
  MicrosoftClientSecret: values.MicrosoftClientSecret,
...
export function OAuthSection(props: OAuthSectionProps) {
  ...
  const microsoftCallbackUrl = buildOAuthCallbackUrl(
    props.serverAddress,
    'microsoft',
    t('Site URL')
  )
          ...
          <SettingsForm onSubmit={form.handleSubmit(onSubmit)}>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              <TabsList className='grid w-full grid-cols-4 md:grid-cols-8'>
                ...
                <TabsTrigger value='microsoft'>{t('Microsoft')}</TabsTrigger>
              ...
              >
                ...
                <FormField
                  control={form.control}
                  name='MicrosoftOAuthEnabled'
                  render={({ field }) => (
                    <SettingsSwitchItem>
                      <SettingsSwitchContent>
                        <FormLabel>{t('Enable Microsoft OAuth')}</FormLabel>
                        <FormDescription>
                          {t('Allow users to sign in with Microsoft')}
                        </FormDescription>
                      </SettingsSwitchContent>
                      <FormControl>
                        <Switch
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          placeholder={t('Your Microsoft OAuth Client ID')}
                          autoComplete='off'
                          value={field.value ?? ''}
                          onChange={(event) =>
                            field.onChange(event.target.value)
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          type='password'
                          placeholder={t('Your Microsoft OAuth Client Secret')}
                          autoComplete='new-password'
                          value={field.value ?? ''}
                          onChange={(event) =>
                            field.onChange(event.target.value)
